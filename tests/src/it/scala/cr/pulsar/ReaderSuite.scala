...
object ReaderSuite extends NeutronSuite {
  (1 to 200).toList.foreach { i =>
    test("Reader can check if topic has messages " + i) { client =>
      val topic = mkTopic
      println(s"Topic1 ${i}: " + topic.url.value)
      val event = mkEvent
        ...
        case (producer, reader) =>
          for {
            ...
            _ <- if (res1 == MessageAvailable.Yes) reader.read1.flatMap { e =>
                  IO.println(s"${i} Received unexpected event: ${e}, expected: ${event}")
                }
                else IO.println(s"${i} As expected")
